...
if (actBtn) {
  actBtn.addEventListener("click", async function () {
    ...
    if (!actInput.trim()) {
      ...
      actOutput.innerText = "⚠️ Please describe your problem first!";
      return;
    }

    // Show loading
    actOutput.style.color = "#5C6C85";
    actOutput.innerText = "✨ Getting expert advice...";
...
Return ONLY the advice. No extra text or intro.`
      ...
      actOutput.innerText = data.choices[0].message.content;

    } catch (error) {
      actOutput.style.color = "#ff4757";
      actOutput.innerText = "Something went wrong. Make sure your API key is correct.";
      console.error(error);
    }
...
function copyActOutput() {
  ...
  actCopyBtn.innerText = "Copied!";

  setTimeout(function () {
    actCopyBtn.innerText = "Copy";
  }, 2000);
}
